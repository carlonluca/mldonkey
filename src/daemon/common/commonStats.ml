...
let print_stats_ascii buf arr l tl uptime =
  ...
  for i = 0 to ((Array.length arr)-1) do
    ...
    if r.brand_seen > 0 then
      Printf.bprintf buf "%-12s|%9d %3.0f%%|%9.1f %5.1f %3.0f%%|%9.1f %5.1f %3.0f%%|%7d %3.0f%%|%9d %3.0f%%\n"
  ...
  for i=0 to (Array.length a) - 1 do
    ...
    begin
      ...
      html_mods_td buf [
        ...
        ("", (if is_total then "srp total" else "srp"), Printf.sprintf "(%.0f%%)" seen_percent);
        ("", (if is_total then "sr total" else "sr"), "|");
        ("", (if is_total then "sr ar total" else "sr ar"), Printf.sprintf "%d" r.brand_filerequest);
        ("", (if is_total then "srp total" else "srp"), Printf.sprintf "(%.0f%%)" freq_percent);
        ("", (if is_total then "sr total" else "sr"), "|");
        ("", (if is_total then "sr ar total" else "sr ar"), Printf.sprintf "%d" r.brand_banned);
        ("", (if is_total then "srp total" else "srp"), Printf.sprintf "(%.0f%%)" banned_percent);
        ("", (if is_total then "sr total" else "sr"), "|");
