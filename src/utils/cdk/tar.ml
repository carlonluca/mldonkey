...
class in_chan i = object
  method really_input str pos len =
    try Stdlib.really_input i str pos len with
        End_of_file -> raise (Error "Unexpected end of file")
  method input str pos len =
    try Stdlib.input i str pos len with
        End_of_file -> raise (Error "Unexpected end of file")
  method dispose () = ()
  method close () = Stdlib.close_in i
end
...
class out_chan o = object
  method output str pos len = Stdlib.output o str pos len
  method flush () = Stdlib.flush o
  method close () = Stdlib.close_out o
end
