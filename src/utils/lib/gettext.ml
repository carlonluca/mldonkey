...
let lprintf_nl fmt = lprintf_nl2 log_prefix fmt
  
let lprintf_n fmt = lprintf2 log_prefix fmt
...
let type_format fmt =
  ...
  let incomplete i =
    failwith
      (Printf.sprintf "Incomplete format %s" (String.sub fmt i (len - i))) in
  let bad_format i j =
    failwith
      (Printf.sprintf "Bad format %s" (String.sub fmt i ((j - i) + 1))) in
  let ty_result = [] in
  ...
  and scan_conversion i j =
    ...
    then incomplete i
      ...
      (match fmt.[j] with
       | '%' -> scan_format (j + 1)
       | 's' | 'S' | '[' -> ty_arrow Type_string (scan_format (j + 1))
       | 'c' | 'C' -> ty_arrow Type_char (scan_format (j + 1))
       | 'd' | 'i' | 'o' | 'x' | 'X' | 'u' | 'N' ->
           ty_arrow Type_int (scan_format (j + 1))
       | 'f' | 'e' | 'E' | 'g' | 'G' ->
           ty_arrow Type_float (scan_format (j + 1))
       | 'b' -> ty_arrow Type_bool (scan_format (j + 1))
       | 'a' -> bad_format i j
       | 't' -> bad_format i j
       | 'l' ->
           if (j + 1) >= len
...
let arrow_add_variable (x : 'a variable) (y : 'b arrow) =
  let x = Obj.magic x in
  let y = Obj.magic y in (Obj.magic (x :: y) : ('a -> 'b) arrow)
...
let value_to_text (expected_type : 'a arrow) v =
  ...
  let format_type = type_format s
  in
    if format_type = expected_type
    then (Obj.magic s : ('a, unit, string) format)
    else failwith "Bad format"
  
let text_to_value v = let v = Obj.magic v in string_to_value v
  
let text_option (expected_type : 'a arrow) =
  define_option_class "Text" (value_to_text expected_type) text_to_value
...
module T =
  struct
    ...
    let string x =
      arrow_add_variable (Obj.magic Type_string : string variable) x
      
    let float x =
      arrow_add_variable (Obj.magic Type_float : float variable) x
      
    let bool x = arrow_add_variable (Obj.magic Type_bool : bool variable) x
      
    let int32 x =
      arrow_add_variable (Obj.magic Type_int32 : int32 variable) x
      
    let int64 x =
      arrow_add_variable (Obj.magic Type_int64 : int64 variable) x
      
    let nativeint x =
      arrow_add_variable (Obj.magic Type_nativeint : nativeint variable) x
      
    let format : string arrow = Obj.magic []
      
    let bformat : unit arrow = Obj.magic []
...
let _ =
  ...
  *)
type 'a _string = { name : string; index : int }
...
let register modname x =
  try Hashtbl.find strings x
  with
  | Not_found -> (*          lprintf "New message [%s]\n" x;  *)
      (save_strings_file := true;
       if !next_slot = (Array.length !translation)
       then (*              lprintf "Incrementing size\n"; *)
               ...
               (Array.blit !translation 0 new_array 0 !next_slot;
                ...
                in
                  (Array.blit !requests 0 new_array 0 !next_slot;
                   requests := new_array;
                   let new_array =
                     Array.make ((2 * !next_slot) + 1) no_translation
                   in
                     (Array.blit !default 0 new_array 0 !next_slot;
                      default := new_array)))))
       else ();
       let index = !next_slot in
       let m = { name = x; index = index; }
       in
         ((try
             let names = Hashtbl.find modules modname
             in names := index :: !names
           with | _ -> Hashtbl.add modules modname (ref [ index ]));
...
let translate modname s t =
  if (t <> "") && (t <> s)
  then (*          lprintf "Register\n"; *)
    (let m = register modname s
     in
       (*          lprintf "Translation: %s = %s\n" s t; *)
       (save_strings_file := true; !translation.(m.index) <- t))
  else ()
...
let verify index translated =
  ...
  let translated_type = type_format translated
  in
    if index_type = translated_type
    then (!verified.(index) <- true; true)
    else
...
let bb_ :
  string -> ('a, 'b, 'c, 'd) format4 -> (('a, 'b, 'c, 'd) format4) _string =
  fun modname -> Obj.magic (ss_ modname)
  
let _bb : (('a, 'b, 'c, 'd) format4) _string -> ('a, 'b, 'c, 'd) format4 =
  fun m ->
    let index = m.index
    in
      (!requests.(index) <- !requests.(index) + 1;
       ...
       let s =
         ...
         then !default.(index)
           ...
           if !verified.(index) || (verify index translation)
           then translation
           else !default.(index)
       in Obj.magic s)
...
let save_strings () =
  ...
  | None -> ()
  | Some filename ->
      if !save_strings_file && (not !strings_file_error)
      then
        (try
           Unix2.tryopen_write filename
             (fun oc ->
                (Hashtbl.iter
                   (fun modname names ->
                      (Printf.fprintf oc
                       ...
                       List.iter
                         (fun i ->
                            Printf.fprintf oc "\"%s\" = \"%s\"\n\n"
                              ...
                              (String.escaped
                                 (if ( != ) !translation.(i) no_translation
                                  then !translation.(i)
                                  else !default.(i))))
                         !names))
                   modules;
         ...
         with
         | e -> lprintf_nl "save_strings: Error %s" (Printexc2.to_string e))
      else ()
...
let rec parse_file (__strm : _ Stream.t) =
      ...
      (Stream.junk __strm;
       (match Stream.peek __strm with
        | Some (String modname) ->
            (Stream.junk __strm;
             let strm = __strm
             in (current_modname := modname; parse_file strm))
        | _ -> raise (Stream.Error "")))
  | _ -> (* lprintf "done\n" *) ()
...
let set_strings_file filename =
  let filename =
    let extension =
      try Unix.getenv "LANG"
      with
      | _ ->
          Charset.Locale.default_language ^
            ("_" ^ Charset.Locale.locale_string)
    in Printf.sprintf "%s.%s" filename extension
  in
             ...
             (lprintf_nl "Loading language resource %s" filename;
              ...
              in
                try
                  ...
                  in
                    (*        lprintf "x\n"; *)
                    (current_modname := "general"; parse_file stream)
                with
                | e ->
                    (strings_file_error := true;
                     lprintf_nl
                       "set_strings_file: Exception %s in %s at pos %d"
                       (Printexc2.to_string e) filename (Stream.count s))))
      with | e -> save_strings_file := true);
     save_strings ())
...
let _ =
  try
    ...
    let translate1 s0 s1 =
      (lprintf_nl "translate0 %s" s0; Hashtbl.add strings s0 s1) in
    let rec parse_file (__strm : _ Stream.t) =
      match Stream.peek __strm with
      | Some (Ident s0) ->
          (Stream.junk __strm;
           (match Stream.peek __strm with
            | Some (Kwd "=") ->
                (Stream.junk __strm;
                 let s1 =
                   (try parse_next __strm
                    with | Stream.Failure -> raise (Stream.Error "")) in
                 let strm = __strm in (translate1 s0 s1; parse_file strm))
            | _ -> raise (Stream.Error "")))
      | _ -> (* lprintf "done\n" *) ()
    and parse_next (__strm : _ Stream.t) =
      match Stream.peek __strm with
      | Some (Ident s1) -> (Stream.junk __strm; s1)
      | Some (String s1) -> (Stream.junk __strm; s1)
      | _ -> raise Stream.Failure
    in
      ((try
          Unix2.tryopen_read f1
            (fun ic ->
               let s = Stream.of_channel ic
               in
                 try
        ...
        | e ->
            (save_strings_file := true;
             lprintf_nl
               "set_strings_file: no message file found. Creating one"));
       let translate2 s0 s1 =
         try
           (lprintf_nl "translate2 %s" s0;
            let s0 = Hashtbl.find strings s0
            in translate "Former Translation" s0 s1)
         with | _ -> lprintf_nl "No translation for %s" s0 in
       let rec parse_file (__strm : _ Stream.t) =
         match Stream.peek __strm with
         | Some (Ident s0) ->
             (Stream.junk __strm;
               ...
               | Some (Kwd "=") ->
                   (Stream.junk __strm;
                    let s1 =
                      (try parse_next __strm
                       with | Stream.Failure -> raise (Stream.Error "")) in
                    let strm = __strm in (translate2 s0 s1; parse_file strm))
               | _ -> raise (Stream.Error "")))
         | _ -> (* lprintf "done\n" *) ()
       and parse_next (__strm : _ Stream.t) =
         match Stream.peek __strm with
