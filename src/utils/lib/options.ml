...
type 'a option_class =
  ...
  and 'a option_record =
  { option_class : 'a option_class; mutable option_value : 'a;
    option_name : string list; mutable option_desc : string;
    option_help : string; option_default : 'a;
    mutable option_hooks : (unit -> unit) list;
    option_section : options_section; option_advanced : bool;
    option_restart : bool; option_public : bool; option_internal : bool
  }
  and options_file =
  { mutable file_name : string; mutable file_sections : options_section list;
    mutable file_rc : option_module; mutable file_pruned : bool;
    mutable file_before_save_hook : unit -> unit;
    mutable file_after_save_hook : unit -> unit;
    mutable file_after_load_hook : unit -> unit
  }
  and options_section =
  { section_name : string list; section_help : string;
...
let file_section file section_name section_help =
  ...
  in (file.file_sections <- file.file_sections @ [ s ]; s)
...
let create_options_file name =
  ...
  in
    (ignore
       (file_section file [ "Header" ] "These options must be read first");
     file)
...
let define_option_class (class_name : string)
...
let find_value list m =
  try find_value list m with | _ -> raise OptionNotFound
  
let prune_file file = file.file_pruned <- true
  
let define_simple_option normalp (section : options_section)
                         (option_name : string list) desc restart public
                         internal (option_help : string)
    ...
    (section.section_options <-
     ...
     o.option_value <-
       (try
        ...
        with | OptionNotFound -> default_value
        | e ->
...
let define_header_option opfile option_name option_help option_class
                         default_value =
  define_simple_option false (List.hd opfile.file_sections) option_name None
    None None None option_help option_class default_value false
...
let define_option opfile option_name ?desc ?restart ?public ?internal
  ...
  define_simple_option true opfile option_name desc restart public internal
    option_help option_class default_value false
  
let define_expert_option opfile option_name ?desc ?restart ?public ?internal
                         option_help option_class default_value =
  define_simple_option true opfile option_name desc restart public internal
    option_help option_class default_value true
...
let rec parse_gwmlrc (__strm : _ Stream.t) =
  match try Some (parse_id __strm) with | Stream.Failure -> None with
  | Some id ->
      (match Stream.peek __strm with
       | Some (Kwd "=") ->
           (Stream.junk __strm;
            let v =
              (try parse_option __strm
               with | Stream.Failure -> raise (Stream.Error "")) in
            let eof =
              (try parse_gwmlrc __strm
               with | Stream.Failure -> raise (Stream.Error ""))
            in (id, v) :: eof)
       | _ -> raise (Stream.Error ""))
  | _ -> []
and parse_option (__strm : _ Stream.t) =
  match Stream.peek __strm with
  | Some (Kwd "{") ->
      (Stream.junk __strm;
       let v =
       ...
       in
         (match Stream.peek __strm with
          | Some (Kwd "}") -> (Stream.junk __strm; Module v)
          | _ -> raise (Stream.Error "")))
  | Some (Ident s) -> (Stream.junk __strm; StringValue s)
  | Some (String s) -> (Stream.junk __strm; StringValue s)
  | Some (Int i) -> (Stream.junk __strm; IntValue i)
  | Some (Float f) -> (Stream.junk __strm; FloatValue f)
  | Some (Kwd "@") ->
      (Stream.junk __strm;
       (match Stream.peek __strm with
        | Some (Int i) ->
            (Stream.junk __strm;
             let v =
               (try parse_once_value i __strm
                with | Stream.Failure -> raise (Stream.Error ""))
             in OnceValue v)
        | _ -> raise (Stream.Error "")))
  | Some (Char c) -> (Stream.junk __strm; StringValue (String.make 1 c))
  | Some (Kwd "[") ->
      (Stream.junk __strm;
       let v =
         (try parse_list [] __strm
          with | Stream.Failure -> raise (Stream.Error ""))
       in List v)
  | Some (Kwd "(") ->
      (Stream.junk __strm;
...
and parse_once_value i (__strm : _ Stream.t) =
  match Stream.peek __strm with
  | Some (Kwd "@") ->
      (Stream.junk __strm;
       (try Hashtbl.find once_values i
        with
        | Not_found ->
  ...
  | Some (Kwd "=") ->
      (Stream.junk __strm;
       let v =
         (try parse_option __strm
          with | Stream.Failure -> raise (Stream.Error ""))
       in (Hashtbl.add once_values i v; v))
  | _ -> raise Stream.Failure
and parse_id (__strm : _ Stream.t) =
  match Stream.peek __strm with
  | Some (Ident s) -> (Stream.junk __strm; s)
  | Some (String s) -> (Stream.junk __strm; s)
  | _ -> raise Stream.Failure
and parse_list list (__strm : _ Stream.t) =
  match Stream.peek __strm with
  | Some (Kwd ";") -> (Stream.junk __strm; parse_list list __strm)
  | Some (Kwd ",") -> (Stream.junk __strm; parse_list list __strm)
  | Some (Kwd ".") -> (Stream.junk __strm; parse_list list __strm)
  | _ ->
      (match try Some (parse_option __strm) with | Stream.Failure -> None
       with
       | Some v -> parse_list (v :: list) __strm
       | _ ->
           (match Stream.peek __strm with
            | Some (Kwd "]") -> (Stream.junk __strm; List.rev list)
            | Some (Kwd ")") -> (Stream.junk __strm; List.rev list)
            | _ -> raise Stream.Failure))
...
let count_lines ic pos =
  (seek_in ic 0;
   ...
   let lines = ref 0
   in
     (while (pos_in ic) < pos do prev := pos_in ic; incr lines;
        ignore (input_line ic) done;
      ((!lines), (pos - !prev))))
  
let really_load filename sections =
  let temp_file = filename ^ ".tmp"
  in
    (if Sys.file_exists temp_file
     then
     ...
     Unix2.tryopen_read filename
       (fun ic ->
          let s = Stream.of_channel ic
          in
            try
              let stream = lexer s
              in
                (Hashtbl.clear once_values;
                 let list =
                   try parse_gwmlrc stream
                   with
                   | e ->
                       let (line, col) = count_lines ic (Stream.count s)
                       in
                         (lprintf
                            "Syntax error while parsing file %s at position %d:%d: %s\n"
                 ...
                 in
                   (Hashtbl.clear once_values;
                    let affect_option o =
                      ...
                      with | SideEffectOption -> ()
                      | OptionNotFound ->
                          if !print_options_not_found
                          then
                            (lprintf "Option ";
                             List.iter (fun s -> lprintf "%s " s)
                               o.option_name;
...
the last defined one ("defined" in the order of the program execution).
  Don't change this. *)
                      (List.iter
                         (fun s -> List.iter affect_option s.section_options)
                         sections;
                       list)))
            with
            | e ->
...
let safe_string s =
  ...
  then "\"\""
    ...
    (try
       ...
       | _ ->
           if
             ((Int64.to_string (Int64.of_string s)) = s) ||
               ((string_of_float (float_of_string s)) = s)
           then s
           else raise exit_exn
     with | _ -> Printf.sprintf "\"%s\"" (String.escaped s))
          ...
          | m :: tail ->
              let p =
                (try List.assoc m !subm
                 with | e -> let p = ref [] in (subm := (m, p) :: !subm; p))
              in p := (tail, help, restart, internal, value) :: !p)
       list;
     List.iter
       (fun (m, p) ->
...
and save_value indent oc v =
  ...
  | OnceValue v ->
      (try
         let i = Hashtbl.find once_values_rev v
         in Printf.fprintf oc "@%Ld@" i
       with
       | Not_found ->
           (incr once_values_counter;
            let i = Int64.of_int !once_values_counter
            in
...
let load opfile =
  ((try opfile.file_rc <- really_load opfile.file_name opfile.file_sections
...
let append opfile filename =
  try
    opfile.file_rc <-
      (really_load filename opfile.file_sections) @ opfile.file_rc
  with | Not_found -> lprintf "No %s found\n" filename
...
let stringoption_to_value v =
  match v with | None -> StringValue "None" | Some s -> StringValue s
...
let rec value_to_hasharray v2c v =
  match v with
  | List l ->
      let hash = Array.init 256 (fun _ -> Hashtbl.create 10)
      in
        (List.iter
           (fun a ->
  ...
  | List l | SmallList l ->
      let rec iter map left =
        (match left with
         ...
         | x :: tail ->
             let map =
               (try let v = v2c x in let num = f v in Intmap.add num v map
                with | _ -> map)
             in iter map tail)
      in iter Intmap.empty l
...
let rec value_to_listiter v2c v =
  match v with
  | List l | SmallList l ->
      (List.iter (fun v -> try ignore (v2c v) with | SideEffectOption -> ())
         l;
...
let option_to_value c2v o =
  match o with | None -> StringValue "" | Some c -> c2v c
...
let intmap_to_value name c2v map =
  DelayedValue
    (fun oc indent ->
       let save = save_delayed_list_value oc indent c2v
       in
         (Printf.fprintf oc "[";
...
let path_to_value list =
  StringValue
    (Filepath.colonpath_to_string (List.map Filename2.to_string list))
...
let percent_option =
  define_option_class "Int" value_to_percent percent_to_value
...
let value_to_array from_value a = Array.of_list (value_to_list from_value a)
  
let array_to_value to_value v = list_to_value to_value (Array.to_list v)
...
let safelist_option cl =
  define_option_class (cl.class_name ^ " List")
    (value_to_safelist cl.from_value) (list_to_value cl.to_value)
...
let listiter_option cl =
  define_option_class (cl.class_name ^ " List")
    (value_to_listiter cl.from_value) (list_to_value cl.to_value)
...
let sum_option l =
  let ll = List.map (fun (a1, a2) -> (a2, a1)) l
  in define_option_class "Sum" (value_to_sum l) (sum_to_value ll)
  
let option_to_value o =
  ((o.option_name), (o.option_help), (o.option_restart), (o.option_internal),
   (try o.option_class.to_value o.option_value
    with
    | e ->
        (lprintf "Error while saving option \"%s\": %s\n"
           (try List.hd o.option_name with | _ -> "???")
           (Printexc2.to_string e);
         StringValue "")))
...
let string_of_string_list list =
  let rec iter s list =
    match list with
    | [] -> s
    | ss :: tail -> iter (Printf.sprintf "%s.%s" s ss) tail
  in match list with | [] -> "" | s :: tail -> iter s tail
  
let title_opfile = ref true
  
let save opfile =
  (opfile.file_before_save_hook ();
   let old_config_dir = "old_config"
   in
     (if not (Sys.file_exists old_config_dir)
      then Unix.mkdir old_config_dir 0o755
      else ();
      let filename = opfile.file_name in
      let old_file = Filename.concat old_config_dir filename
      in
        try
          (Unix2.with_remove (filename ^ ".tmp")
             (fun temp_file ->
                (Unix2.tryopen_write temp_file
                   (fun oc -> (* race! *)
                      (if !save_private
                       then (try Unix.chmod temp_file 0o600 with | _ -> ())
                       else ();
                       ...
                       in
                         (List.iter
                               ...
                               in
                                 ...
                                 then
                                   (if s.section_name <> []
                                    ...
                                    save_module "" oc
                                      (List.map option_to_value options))
                                 else ())
                          ...
                          then
                            (Printf.fprintf oc "\n\n\n";
                             ...
                             List.iter
                               (fun s ->
                                  let options =
                                    List.filter (fun o -> o.option_advanced)
                                      s.section_options
                                  in
                                    if options = []
                                    then ()
                          ...
                          Hashtbl.clear once_values_rev)));
                 try
                   ((try Unix2.rename filename old_file
                     with | Unix.Unix_error (Unix.ENOENT, _, _) -> ());
                    Unix2.rename temp_file filename)
                 with
                 | e ->
                     lprintf_nl "[Opt] exception %s while saving %s"
                       (Printexc2.to_string e) filename));
           (* remove temp_file *)
           opfile.file_after_save_hook ())
        with | e -> (opfile.file_after_save_hook (); raise e)))
  
let save_with_help opfile =
  (with_help := true; (try save opfile with | _ -> ()); with_help := false)
              ...
              (match o.option_name with
               ...
               | name :: tail ->
                   (Printf.fprintf oc "%s" name;
                    iter_order (fun name -> Printf.fprintf oc ":%s" name)
                      o.option_name));
...
let tuple4_to_value (c1, c2, c3, c4) (a1, a2, a3, a4) =
  SmallList
    [ to_value c1 a1; to_value c2 a2; to_value c3 a3; to_value c4 a4 ]
...
let get_option opfile name = (*  lprintf "get_option [%s]\n" name;*)
  let rec iter name list sections =
    match list with
    | o :: list ->
        if o.option_name = name then o else iter name list sections
    | [] ->
        (match sections with
         | [] ->
             (prerr_endline
              ...
              raise Not_found)
         | s :: tail -> iter name s.section_options tail)
  in iter [ name ] [] opfile.file_sections
  
let set_simple_option opfile name v =
  let o = get_option opfile name
  in
...
let restore_default o = o =:= o.option_default
  
let set_option_desc o s = o.option_desc <- s
  
module M =
  struct
    type option_info =
      { option_name : string; option_shortname : string;
        option_desc : string; option_value : string; option_help : string;
        option_advanced : bool; option_default : string;
        option_type : string; option_restart : bool; option_public : bool;
...
let string_of_option_value o v =
  match o.option_class.string_wrappers with
  | None -> value_to_string (o.option_class.to_value v)
  | Some (to_string, _) -> to_string v
  
let tuple2_to_value f x = let (v1, v2) = f x in SmallList [ v1; v2 ]
...
let strings_of_option prefix o =
  ...
  | [ name ] ->
      let desc = if o.option_desc = "" then name else o.option_desc
      in
        {
  ...
  in
    (List.iter
       (fun s ->
          List.iter
            (fun o ->
               if admin || o.option_public
               then
                 (try list := (strings_of_option prefix o) :: !list
                  with | _ -> ())
               else ())
            s.section_options)
       opfile.file_sections;
           ...
           (fun s ->
              (lprintf_nl "Setting option %s" oi.M.option_name;
               let name =
                 ...
                 | [ _; (* ^ "-" = prefix *) name ] -> name
                 | (* opfile is corresponding to prefix *) _ ->
                     oi.M.option_name
               in set_simple_option opfile name s))),
        (Printf.sprintf "<string> : \t%s (current: %s)" oi.M.option_help
           oi.M.option_value)))
...
let prefixed_args prefix file =
  List.map
    (fun (s, f, h) ->
       ...
       in ((Printf.sprintf "-%s:%s" prefix s), f, h))
    (simple_args "" file)
...
type option_info =
  M.option_info =
    { option_name : string; option_shortname : string; option_desc : string;
      option_value : string; option_help : string; option_advanced : bool;
      option_default : string; option_type : string; option_restart : bool;
      option_public : bool; option_internal : bool
    }

let iter_section f s = List.iter f s.section_options
  
let iter_file f file = List.iter (iter_section f) file.file_sections
  
let strings_of_option o = strings_of_option "" o
  
let set_after_load_hook file f = file.file_after_load_hook <- f
  
let set_after_save_hook file f = file.file_after_save_hook <- f
